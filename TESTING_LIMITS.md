...
| **DCR delete + re-issue** | 30 cycles | clean at ceiling under the pre-repair allocator; suite updated to the immutable-number policy (max+1, deleted numbers stay retired — no gap-fill reuse) | >30 cycles unverified under the new allocator | `next_dcr_sequence` is strictly MAX(seq)+1 since the repair pass (issue never renumbers an existing report). See `tests/smoke_dcr_volume.py` phase 2. |
